...
function UsersList (p) {
  ...
  return (
    ...
    <div className="flex items-center justify-center pt-12">
      {!users.length
        ? (<div><p className="py-72 h-[50%]">No hay usuarios registrados.</p></div>)
          ...
          {users.map((user, index) => {
            return (
              <div key={index} className='border-[1px] border-gray-300 m-5 p-5 rounded-md max-sm:m-2 max-sm:text-sm max-sm:p-4'>
                ...
                <button className="bg-red-600 w-full font-bold text-lg text-gray-100 py-2 mt-5 rounded-lg hover:bg-red-500 transition-colors max-sm:text-sm max-sm:mt-4"
